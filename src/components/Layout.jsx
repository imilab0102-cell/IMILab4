...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col lg:block">
      ...
      {/* Mobile header */}
      <header className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-card/95 backdrop-blur-md border-b border-border safe-area-inset-top">
      ...
      <main className="lg:pl-64 pt-14 lg:pt-0 flex-1 safe-area-inset-bottom">
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-card border-t border-border flex justify-around items-center h-16 pb-safe z-50">
        {links.filter(link => ['/', '/orders', '/clinics', '/doctors', '/external-lab'].includes(link.to)).slice(0, 5).map(link => {
          const Icon = link.icon;
          const active = location.pathname === link.to;
          return (
            <Link
            ...
            >
              ...
              <span className="text-[10px] mt-1 font-medium">{link.label.split(' ')[0]}</span>
            </Link>
          );
        })}
      </nav>
